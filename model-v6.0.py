...
class RetroReader(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        verifier_logits = self.verifier_head(cls_token).squeeze(-1)

        if self.is_squad_v2:
            return start_logits, end_logits, answer_logits, verifier_logits
        else:
            return start_logits, end_logits, verifier_logits
...
def prepare_training_data_squad_v1(tokenizer, max_length=384):
    ...
    def preprocess(examples):
        ...
        inputs["verifier_labels"] = verifier_labels
        return inputs
...
def train_model_and_save(
    ...
    for epoch in range(epochs):
        ...
        for batch in train_bar:
            ...
            verifier_labels = batch["verifier_labels"].to(device)
            ...
            if is_squad_v2:
                ...
                start_logits, end_logits, verifier_logits = model(
                    input_ids=input_ids,
                    attention_mask=attention_mask
                )
                answerable_loss = 0
            ...
            verifier_loss = nn.CrossEntropyLoss()(verifier_logits, verifier_labels.float())
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                val_loss = start_loss + end_loss + answerable_loss + verifier_loss
...
def load_model_and_predict(
    ...
    for batch in tqdm(val_loader, desc="Generating predictions"):
        ...
        for i in range(input_ids.size(0)):
            ...
            verifier_score = torch.sigmoid(verifier_logits[i]).item()
